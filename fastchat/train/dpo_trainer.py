...
from trl.trainer.utils import DPODataCollatorWithPadding, disable_dropout_in_model, pad_to_length, trl_sanitze_kwargs_for_tagging
...
class DPOMultiTrainer(Trainer):
    ...
    def dpo_with_sft_loss(
        ...
        if self.loss_type == "sigmoid":
            losses = (
                -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits) * self.label_smoothing - policy_chosen_logps
            )
        elif self.loss_type == "hinge":
            losses = torch.relu(1 - self.beta * logits)
        elif self.loss_type == "ipo":
    ...
    def get_batch_loss_metrics(
        ...
        losses, chosen_rewards, rejected_rewards = self.dpo_with_sft_loss(
            policy_chosen_logps,
            policy_rejected_logps,
            reference_chosen_logps,
            reference_rejected_logps,
