...
class ScriptArguments:
    ...
    data_path: str = field(
        default="data_pm/feishu_travel_ppo.json",
...
def filter_batch(batch):
    ...
    new_batch = {}
    new_batch["prompt_input_ids"] = []
    new_batch["response_input_ids"] = []
    new_batch["reward"] = []
    new_batch["response_mask"] = []
    for i in range(len(response_input_ids)):
        if len(response_input_ids[i]) > 0:
            new_batch["prompt_input_ids"].append(prompt_input_ids[i])
            new_batch["response_input_ids"].append(response_input_ids[i])
            new_batch["reward"].append(reward[i])
            new_batch["response_mask"].append(response_mask[i])
    return new_batch
...
def train():
    ...
    # Load data
    dataset = load_dataset("json", data_files=args.data_path, cache_dir="data_cache")
