# PRM LoRA FP16 training for Feishu CUA trajectories.
...
TRAINING_OUTPUT_DIR = os.getenv("PRM_OUTPUT_DIR", "./records/progress_model_feishu_lora_fp16")
...
lora_config = LoraConfig(
    ...
    target_modules=["q_proj", "v_proj"],  # 只对注意力的Q/V投影加LoRA
    lora_dropout=0.05,
    bias="none",
    task_type="CAUSAL_LM"
)
# 包装为 LoRA 模型
lora_model = get_peft_model(base_model, lora_config).to(torch.float16)
...
class prm_model(PreTrainedModel):
    def __init__(self, base_model, vocab_size=32000):
        ...
        self.LN = nn.Linear(vocab_size, 1).to(torch.float16)
    ...
    def forward(self, input_ids, attention_mask, gpt_unmask=None, labels=None):
        ...
        if torch.all(value_outputs == 0):
            print("Warning: value_outputs 全是 0")
